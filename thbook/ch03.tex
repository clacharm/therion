...
|layout <id> [OPTIONS]
    ...
    statistics <explo-length/topo-length> <on/hide/off>
    statistics <carto-count/copyright-count> <on/hide/off>
    scale-bar <length> <units>
    survey-level <number/all>
    language <xx[_YY]>
...
\fi
  ...
  * |statistics <explo-length/topo-length> <on/hide/off>| = display some basic
    ...
    (M, A; default: |hide|)
  * |statistics <carto-count/copyright-count> <on/hide/off>| = \NEW{5.5.4} display number of scraps
  ...
  * |survey-level <number/all>| = the number of survey levels displayed
